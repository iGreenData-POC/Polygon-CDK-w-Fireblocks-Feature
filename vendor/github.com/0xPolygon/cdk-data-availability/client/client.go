...
type Client interface {
	...
	SignSequence(signedSequence types.SignedSequence, fireblocksFeatureEnabled bool, rawSigningAdaptorUrl string) ([]byte, error)
}
...
func (c *client) SignSequence(signedSequence types.SignedSequence, fireblocksFeatureEnabled bool, rawSigningAdaptorUrl string) ([]byte, error) {
	response, err := rpc.JSONRPCCall(c.url, "datacom_signSequence", signedSequence, fireblocksFeatureEnabled, rawSigningAdaptorUrl)
	if err != nil {
		return nil, err
	}
